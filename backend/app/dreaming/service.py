...
def _persona_display_name(persona: str) -> str:
    """Map persona key to human-readable display name."""
    names = {
        "locutus": "Locutus",
        "seven": "Seven of Nine",
...
async def _create_dream_note(
    ...
    Args:
        ...
        run: The DreamingRun record
        persona: Optional persona key for branding (e.g. 'locutus', 'seven')
    """
    display_name = _persona_display_name(persona) if persona else "BorgOS"
    lines = [
        f"# {display_name}'s Dream Diary — {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')}",
        "",
        f"Generated by {display_name}'s dreaming system (run #{run.id}).",
        "",
        "## Summary",
        "",
